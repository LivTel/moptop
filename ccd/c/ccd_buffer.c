...
/**
 * This hash define is needed before including stdlib.h to give us a function prototype for aligned_alloc.
 */
...
struct Buffer_Struct
{
	...
	AT_U8 *Image_Buffer_List[BUFFER_COUNT];
};
 ...
 * <li>We call CCD_Setup_Dimensions with binning 1 to configure the Andor library. This also sets up the setup
...
int CCD_Buffer_Initialise(void)
{
	...
	/* save the andor library computed image size in bytes. This takes account of the binning (set
	** to 1 above, and extra space for associated meta-data we request in CCD_Setup_Startup */
	...
	for(i=0; i < BUFFER_COUNT; i++)
	{
		Buffer_Data.Image_Buffer_List[i] = aligned_alloc(8,Buffer_Data.Image_Size_Bytes);
		...
		** address (for free).
		...
		CCD_General_Log_Format(LOG_VERBOSITY_VERY_VERBOSE,
				       "CCD_Buffer_Initialise: Buffer %d = %p (8 bit aligned %p).",
				       i,Buffer_Data.Image_Buffer_List[i],
				       (unsigned char*)(((unsigned long)Buffer_Data.Image_Buffer_List[i]) & ~0x7));
#endif /* LOGGING */
	}/* end for */
#if LOGGING > 0
...
/**
 * Add a sequence of image buffers to the Andor library queue.
 * @param image_count The number of images we expect to acquire, we add this number of buffers to the Andor queue.
 * @see ccd_general.html#CCD_General_Log_Format
 * @return The routine returns TRUE on success and FALSE if an error occurs.
 * @see #BUFFER_COUNT
 * @see #Buffer_Error_Number
