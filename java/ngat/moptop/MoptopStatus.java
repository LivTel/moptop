...
/**
 * This class holds status information for the Moptop program.
 * @author Chris Mottram
 * @version $Revision$
 */
public class MoptopStatus
{
	/**
	...
	/**
	 * Default filename containing network properties for moptop.
	 */
	private final static String DEFAULT_NET_PROPERTY_FILE_NAME = "./moptop.net.properties";
	/**
	 * Default filename containing properties for moptop.
	 */
	private final static String DEFAULT_PROPERTY_FILE_NAME = "./moptop.properties";
	/**
	 * Default filename containing FITS properties for moptop.
	 */
	...
	/**
	 * A list of properties held in the properties file. This contains configuration information in moptop
	...
	/**
	 * The name of the ngat.phase2.MoptopPolarimeterConfig object instance that was last used	
	 * to configure the instrument (via an ngat.message.ISS_INST.CONFIG message).
	 * Used for the CONFNAME FITS keyword value.
	 * Initialised to 'UNKNOWN', so that if we try to take a frame before configuring the Moptop
	...
	/**
	 * Set the logging level for Moptop.
	...
	/**
	 * Get the logging level for Moptop.
	...
	/**
	 * Get the the command Moptop is currently processing.
	...
	/**
	 * Method to change (increment) the unique ID number of the last ngat.phase2.MoptopConfig instance to 
	 * successfully configure the Moptop camera.
	 ...
	 * ngat.phase2.MoptopConfig instance to successfully configure the Moptop camera.
	 ...
	 * ngat.phase2.MoptopPolarimeterConfig instance to successfully configure the Moptop camera.
	 * @return The string identifier, or null if the Moptop camera has not been configured
	 * 	since Moptop started.
	 * @see #configName
	 */
	 ...
	 * The value is retrieved from the <b>moptop.thread.priority.server</b> property.
	 * If this fails the default MOPTOP_DEFAULT_THREAD_PRIORITY_SERVER is returned.
	 * @return A valid thread priority between threads MIN_PRIORITY and MAX_PRIORITY.
	 * @see MoptopConstants#MOPTOP_DEFAULT_THREAD_PRIORITY_SERVER
	 */
	public int getThreadPriorityServer()
	{
		...
		try
		{
			retval = getPropertyInteger("moptop.thread.priority.server");
		...
		{
			retval = MoptopConstants.MOPTOP_DEFAULT_THREAD_PRIORITY_SERVER;
		}
		return retval;
	}
	 ...
	 * The value is retrieved from the <b>moptop.thread.priority.interrupt</b> property.
	 * If this fails the default MOPTOP_DEFAULT_THREAD_PRIORITY_INTERRUPT is returned.
	 * @return A valid thread priority between threads MIN_PRIORITY and MAX_PRIORITY.
	 * @see MoptopConstants#MOPTOP_DEFAULT_THREAD_PRIORITY_INTERRUPT
	 */
	public int getThreadPriorityInterrupt()
	{
		...
		try
		{
			retval = getPropertyInteger("moptop.thread.priority.interrupt");
			if(retval < Thread.MIN_PRIORITY)
				retval = Thread.MIN_PRIORITY;
			if(retval > Thread.MAX_PRIORITY)
				retval = Thread.MAX_PRIORITY;
		...
		{
			retval = MoptopConstants.MOPTOP_DEFAULT_THREAD_PRIORITY_INTERRUPT;
		}
		return retval;
	}
	 ...
	 * The value is retrieved from the <b>moptop.thread.priority.normal</b> property.
	 * If this fails the default MOPTOP_DEFAULT_THREAD_PRIORITY_NORMAL is returned.
	 * @return A valid thread priority between threads MIN_PRIORITY and MAX_PRIORITY.
	 * @see MoptopConstants#MOPTOP_DEFAULT_THREAD_PRIORITY_NORMAL
	 */
	public int getThreadPriorityNormal()
	{
		...
		try
		{
			retval = getPropertyInteger("moptop.thread.priority.normal");
			if(retval < Thread.MIN_PRIORITY)
				retval = Thread.MIN_PRIORITY;
			if(retval > Thread.MAX_PRIORITY)
				retval = Thread.MAX_PRIORITY;
		...
		{
			retval = MoptopConstants.MOPTOP_DEFAULT_THREAD_PRIORITY_NORMAL;
		}
		return retval;
	}
	 ...
	 * The value is retrieved from the <b>moptop.thread.priority.tit</b> property.
	 * If this fails the default MOPTOP_DEFAULT_THREAD_PRIORITY_TIT is returned.
	 * @return A valid thread priority between threads MIN_PRIORITY and MAX_PRIORITY.
	 * @see MoptopConstants#MOPTOP_DEFAULT_THREAD_PRIORITY_TIT
	 */
	public int getThreadPriorityTIT()
	{
		...
		try
		{
			retval = getPropertyInteger("moptop.thread.priority.tit");
			if(retval < Thread.MIN_PRIORITY)
				retval = Thread.MIN_PRIORITY;
			if(retval > Thread.MAX_PRIORITY)
				retval = Thread.MAX_PRIORITY;
		...
		{
			retval = MoptopConstants.MOPTOP_DEFAULT_THREAD_PRIORITY_TIT;
		}
		return retval;
	}
	 ...
	 * This is got from the <i>moptop.config.unique_id_filename</i> property.
	...
	private void initialiseConfigId()
	{
		...
		fileName = getProperty("moptop.config.unique_id_filename");
