// MoptopTCPServerConnectionThread.java
// $Header$
package ngat.moptop;
...
public class MoptopTCPServerConnectionThread extends TCPServerConnectionThread
{
	 ...
	 * This can be retrieved from the moptop main object, which has a Hashtable with mappings between
	 * ngat.message class names and implementations of these commands.
	 * @see JMSCommandImplementation
	 * @see Moptop#getImplementation
	 */
	 ...
	 */
	public MoptopTCPServerConnectionThread(Socket connectionSocket)
	{
	 ...
	 * stored in the Moptop object. It sets up the implementation objects references to the Moptop main
	 ...
	 * @see Moptop#getImplementation
	 * @see CommandImplementation#setMoptop
	...
	// set the threads priority
		if(command instanceof INTERRUPT)
			this.setPriority(moptop.getStatus().getThreadPriorityInterrupt());
		else
			this.setPriority(moptop.getStatus().getThreadPriorityNormal());
	// get the implementation - this never returns null.
		commandImplementation = moptop.getImplementation(command.getClass().getName());
	// initialises the command implementations response
		((CommandImplementation)commandImplementation).setMoptop(moptop);
	...
	// setup a generic done object until the command specific one is constructed.
		...
		if(command == null)
		{
			moptop.error("processCommand:command was null.");
			done.setErrorNum(MoptopConstants.MOPTOP_ERROR_CODE_BASE+100);
			done.setErrorString("processCommand:command was null.");
			done.setSuccessful(false);
			return;
		}
		moptop.log(Logging.VERBOSITY_VERY_TERSE,"Command:"+command.getClass().getName()+" Started.");
	...
	// the status's current command is reset to null.
		catch(Exception e)
		{
			...
			moptop.error(s,e);
			done.setErrorNum(MoptopConstants.MOPTOP_ERROR_CODE_BASE+102);
			done.setErrorString(s+e);
			done.setSuccessful(false);
		}
	// change Moptop status once command has been done
		if(!(command instanceof INTERRUPT))
		{
	...
	// log command/done
		moptop.log(Logging.VERBOSITY_VERY_TERSE,"Command:"+command.getClass().getName()+
			" Completed.");
		moptop.log(Logging.VERBOSITY_TERSE,"Done:"+done.getClass().getName()+
