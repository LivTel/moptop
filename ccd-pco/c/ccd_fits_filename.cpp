...
/**
 * Data type holding local data about fits filenames associated with ONE of the cameras on Moptop. 
 ...
 * @see ccd_general.html#CCD_GENERAL_ERROR_STRING_LENGT
 */
...
int CCD_Fits_Filename_Initialise(char instrument_code,const char *data_dir)
{
	...
	{
#if LOGGING > 9
		CCD_General_Log_Format(LOG_VERBOSITY_VERY_VERBOSE,"CCD_Fits_Filename_Initialise:Filename %d is %s.",
...
#if LOGGING > 9
				CCD_General_Log_Format(LOG_VERBOSITY_VERY_VERBOSE,
						       "CCD_Fits_Filename_Initialise:Filename %s has date number %d.",
		...
		{
#if LOGGING > 9
			CCD_General_Log_Format(LOG_VERBOSITY_VERY_VERBOSE,"CCD_Fits_Filename_Initialise:"
 ...
 * @see #Fits_Filename_Error_String
 */
int CCD_Fits_Filename_List_Add(char *filename,char ***filename_list,int *filename_count)
{
	if(filename == NULL)
	{
		Fits_Filename_Error_Number = 9;
		sprintf(Fits_Filename_Error_String,"CCD_Fits_Filename_List_Add:filename is NULL.");
		return FALSE;
	}
	if(filename_list == NULL)
	{
		Fits_Filename_Error_Number = 10;
		sprintf(Fits_Filename_Error_String,"CCD_Fits_Filename_List_Add:filename_list is NULL.");
		return FALSE;
	}
	if(filename_count == NULL)
	{
		Fits_Filename_Error_Number = 11;
		sprintf(Fits_Filename_Error_String,"CCD_Fits_Filename_List_Add:filename_count is NULL.");
		return FALSE;
	}
	if((*filename_list) == NULL)
	{
	...
	if((*filename_list) == NULL)
	{
		...
		sprintf(Fits_Filename_Error_String,
			"CCD_Fits_Filename_List_Add:failed to reallocate filename_list(%d).",(*filename_count));
		return FALSE;
	}
	...
	if((*filename_list)[(*filename_count)] == NULL)
	{
		...
		sprintf(Fits_Filename_Error_String,
			"CCD_Fits_Filename_List_Add:failed to duplicate filename (%s,%d).",filename,(*filename_count));
		return FALSE;
	}
	(*filename_count)++;
	return TRUE;
 ...
 * @see #Fits_Filename_Error_String
 */
int CCD_Fits_Filename_List_Free(char ***filename_list,int *filename_count)
{
	int i;
	...
	if(filename_list == NULL)
	{
		...
		sprintf(Fits_Filename_Error_String,"CCD_Fits_Filename_List_Free:filename_list is NULL.");
		return FALSE;
	}
	if(filename_count == NULL)
	{
		Fits_Filename_Error_Number = 16;
		sprintf(Fits_Filename_Error_String,"CCD_Fits_Filename_List_Free:filename_count is NULL.");
		return FALSE;
	}
...
int CCD_Fits_Filename_Lock(char *filename)
{
	...
	if(!Fits_Filename_Lock_Filename_Get(filename,lock_filename))
		return FALSE;
#if LOGGING > 9
	CCD_General_Log_Format(LOG_VERBOSITY_VERY_VERBOSE,"CCD_Fits_Filename_Lock:Creating lock file %s.",
			       lock_filename);
#endif
	...
	** Note atomic creation probably fails on NFS systems. */
	...
	close(fd);
#if LOGGING > 9
	CCD_General_Log_Format(LOG_VERBOSITY_VERY_VERBOSE,"CCD_Fits_Filename_Lock:Lock file %s created.",
			       lock_filename);
#endif
	return TRUE;
}
...
int CCD_Fits_Filename_UnLock(char *filename)
{
	...
	{
#if LOGGING > 9
		CCD_General_Log_Format(LOG_VERBOSITY_VERY_VERBOSE,"CCD_Fits_Filename_UnLock:Removing lock file %s.",
				       lock_filename);
#endif
		/* remove lock file */
...
#if LOGGING > 9
		CCD_General_Log_Format(LOG_VERBOSITY_VERY_VERBOSE,"CCD_Fits_Filename_UnLock:Lock file %s removed.",
				       lock_filename);
#endif
	}
	return TRUE;
 ...
 * @return Returns TRUE if the routine succeeds and returns FALSE if an error occurs.
 * ccd_general.html#CCD_GENERAL_ONE_MICROSECOND_NS
 */
static int Fits_Filename_Get_Date_Number(int *date_number)
{
