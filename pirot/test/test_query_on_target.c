 ...
 * @see ../cdocs/pirot_command.html#PIROT_Command_Query_ONT
 */
int main(int argc, char *argv[])
{
	...
	PIROT_Set_Log_Handler_Function(PIROT_Log_Handler_Stdout);
        /* open device */
	PIROT_Log(LOG_VERBOSITY_TERSE,"test_query_on_target : Connecting to controller.");
	...
	if(on_target)
		fprintf(stdout,"test_query_on_target:The rotator is on target.\n");
	else
		fprintf(stdout,"test_query_on_target:The rotator is NOT on target.\n");
	fprintf(stdout,"test_query_on_target:Closing connection.\n");
	PIROT_USB_Close();
	return 0;
}
...
static void Help(void)
{
	fprintf(stdout,"Test Query On Target:Help.\n");
	fprintf(stdout,"This program tries to gets whether the Physik Instrumente rotator is currently 'on target' (at it's target position).\n");
	fprintf(stdout,"test_query_on_target -device_name <USB device> [-help]\n");
